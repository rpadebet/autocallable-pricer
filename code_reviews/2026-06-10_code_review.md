...
# Re-Review Addendum (2026-06-10, after fix commits 42b613d…c3bdd18)

## Verified fixed
F2 (KI now terminal-only in all three pricers), F4 (closed-form sign), F5 (FDM Greeks slice), F8 (Step-Down schedule 12/18, 0-based), F9 (`call_price`), Digital maturity coupon at 100% (consistent everywhere incl. `terminal_payoff`), PDE per-obs coupons (correct in both heat-equation and local-vol paths), S2 (`n_steps` wired), S4 (Greeks tab guard), S5 (banner key), S8 (dup import).

## NEW BUG (critical): Standard MC per-obs coupon implementation erases accumulated coupons
`app/mc_standard.py:414` and `:440` both use `=` assignment at a path's final resolution:
- `payoffs[triggered] = (redemption + coupon)*disc` wipes all previously accumulated per-obs coupons for called paths, and line 419 (`payoffs[active] += coupon`) then re-adds the call-date coupon because `active` still includes `triggered` → callers get redemption + **2×** call-date coupon and **0** prior coupons.
- `payoffs[survived] = payoffs_surv` wipes survivors'’ entire coupon stream, replacing it with notional + maturity coupon.

**Measured:** Phoenix @ 200K paths: Standard MC = **$974.00** vs hand-rolled correct reference **$990.70** (±0.23) and FD **$989.40** → −$16.7 bias (~60σ).
**Tests now fail:** replicating the assertions, `test_mc_methods_agree` (diff 18.78 > tol 11.11) and `test_fd_vs_mc_consistency` (diff 19.26 > tol 7.61) both FAIL. The 110/110 in test_results predates these commits.
**Fix:** pay the coupon first to all `active` paths, then `payoffs[triggered] += redemption*disc` (no coupon, `+=`), and make the terminal block `+=` principal-only (`np.where(ki, KI_loss, notional)*disc` — no coupon; the final-obs loop already paid it).

## NEW BUG (small): Survival MC double-counts the maturity coupon
The final-observation per-obs coupon block (scalar `mc_survival.py:358–361`, vectorised `:520–527`) AND the terminal block (`:382–383`, `:544–551`) both pay the maturity coupon. Measured: Survival MC $992.33 vs reference $990.70 → +$1.6 bias. Scalar and vectorised paths agree with each other (both double-count).
**Fix:** remove the coupon term from the european_ki terminal branch (keep it in the per-obs block).

## Still open from the original review
- **F6** Product Builder per-period coupon ×12 (`06_Product_Builder.py:357–362, 447`) — use `ac.coupon_per_period()`.
- **F7** Product Builder step-down indices 1-based vs pricer 0-based, and at-vs-from semantics (`:202, :439`).
- **F10** `call_probabilities` independence approximation; captions still say "analytical — no MC noise" (`02_Pricer.py:873`) / "From the analytical formula in Paper 1 §2.2" (`05_Scenarios.py:287`).
- **Local-vol PDE maturity BC never fires:** `pde_pricer.py:682` condition `t_current <= t_obs < t_axis_phys[step]` is False for `t_obs = T` at step 0 (`T < T`). With per-obs coupons now delivered through this BC, the local-vol FD misses both the maturity call redemption and the maturity coupon. Fix: treat step 0 inclusively (e.g. `t_obs <= t_axis_phys[step]` when `step == 0`, or pre-apply the maturity BC to the terminal condition).
- **S1** antithetic: n_paths silently doubled; SE treats 2N correlated samples as independent (average pairs instead).
- **S3** Pricer-page Courant display uses wrong dx (`abs(x_min)+5` vs actual domain `2−x_min`).
- **S7** dead `call_counts` code; **S10–S12** minor items unchanged.
