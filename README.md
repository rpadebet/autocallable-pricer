...
A pricing and analytics platform for autocallable structured products, built as a technical demonstration for the **MerQube Index Researcher — Options** role.
...
The app uses **pre-collected static snapshots** of SPX options chains (in `sample_data/`). There are no live API calls at runtime — this makes every run reproducible and eliminates demo-day failures. Snapshots were collected via `scripts/collect_snapshot.py` using yfinance during market hours.
